...
# [JAMES] - there could be a risk here that e.g. a single gene symbol has
# more than one ENSG ID? Should check if a key-value pair are going to be
# over written and either print an alert or raise exception
...
for line in lines:
	...
	###if line.startswith("Gene stable ID")==False: #ignore the header
		...
		numbernottoadd=[]
		index_2=0
		# Goal here is to fin dthe index positions of consequences to skip
		...
		if len(newgene.mutation_depth)>0: # CALCULATE THE 4th COMPONENT OF THE SCORE, integrating CONSERVATION SCORE,IMPACT PREDICTION and MUTATION DEPTH
			for a in range(0,len(newgene.mutation_studies)):
				if newgene.mutation_impact[a]=="HIGH":
					newgene.mutation_impact_list.append((newgene.mutation_phastcons[a]/5)+(newgene.mutation_pred_rankscore[a]/5)+0.4)
				elif newgene.mutation_impact[a]=="MODERATE":
					newgene.mutation_impact_list.append((newgene.mutation_phastcons[a] / 5) + (newgene.mutation_pred_rankscore[a] / 5) + 0.3)
				elif newgene.mutation_impact[a]=="LOW":
					newgene.mutation_impact_list.append((newgene.mutation_phastcons[a] / 5) + (newgene.mutation_pred_rankscore[a] / 5) + 0.15)
				else:
					newgene.mutation_impact_list.append((newgene.mutation_phastcons[a] / 5) + (newgene.mutation_pred_rankscore[a] / 5) + 0.10)
				'''
				The above lines have had '' removed. Original from Bugra is below
					if newgene.mutation_impact[a]=="HIGH":
...
raw_log=open("raw_score_components.txt","w")
raw_log.write("EnsemblID\tGeneName\tStudyName\tpercentage_of_cohort_with_mut_inthisgene\tmost_freq_mut_percentage_in_cohort\tnormalised_study_burden_percentages\tnormalised_study_percentage_of_recurrent_mutations\tgene.median_impact_score\tmutation_overall_score\n")


output_file=open("Calculated_scores_for_CANSAR_v2.txt","w")
	...
	for ind in range(0, len(studylist)):
		gene.mutation_overall_score.append(
			(gene.percentage_of_cohort_with_mut_inthisgene[ind]+1) * (gene.most_freq_mut_percentage_in_cohort[ind]) * (1.85 - (
					...
					ind])) * (gene.median_impact_score+1)) # what do the values of 1.85 and 1 do?
		...
		output_file.write("\n")
		# [JAMES] output for debuging
		raw_log.write("%s\t%s\t%s\t%1.3f\t%1.3f\t%1.3f\t%1.3f\t%1.3f\t%1.3f\n" % (
			gene.ensemblt_id,
			gene.genename,
			studylist[ind],
			gene.percentage_of_cohort_with_mut_inthisgene[ind],
			gene.most_freq_mut_percentage_in_cohort[ind],
			normalised_study_burden_percentages[ind],
			normalised_study_percentage_of_recurrent_mutations[ind],
			gene.median_impact_score,
			gene.mutation_overall_score[ind]))
	counter=counter+1
output_file.close()
raw_log.close()
